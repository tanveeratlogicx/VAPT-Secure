...
`;
  ...
  const BuildGenerator = ({ domains, features, activeFile, setAlertState }) => {
    ...
    useEffect(() => {
      ...
      // selectedDomain is still needed below for importedAt sync
      const selectedDomain = (Array.isArray(domains) ? domains : []).find(d => d.domain === buildDomain);

      // Count only features in "Release" state — domain-enabled state is irrelevant for this count
      ...
      const desc = `${whiteLabel.name} Build for ${buildDomain || 'Universal Scope'} is a specialized Vulnerability Assessment and Penetration Testing (VAPT) security hardening package providing comprehensive defense against OWASP Top 10 vulnerabilities. ` +
        ...
        el('div', { className: 'vapt-license-card' }, [
          ...
          el('div', { style: { display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '15px' } }, [
            el('div', { style: { flex: 2 } }, el(TextControl, {
              ...
              placeholder: formState.domain === '*' ? __('Universal — applies to any domain', 'vaptsecure') : '',
            ...
            el('div', { style: { flex: 1, minWidth: '120px' } }, el(SelectControl, {
              ...
              options: [
                ...
                { label: __('Universal (Any Domain)', 'vaptsecure'), value: 'universal' }
              ],
              disabled: isSaving,
              onChange: (val) => {
                if (val === 'universal') {
