...
class VAPTSECURE_Hook_Driver
{
    ...
    private static function get_catalog_data($key)
    {
        if (self::$catalog_data === null) {
            ...
            $active_file = defined("VAPTSECURE_ACTIVE_DATA_FILE")
                ? constant("VAPTSECURE_ACTIVE_DATA_FILE")
                : get_option(
                    "vaptsecure_active_feature_file",
                    "Feature-List-99.json",
                );
            $path =
                VAPTSECURE_PATH . "data/" . sanitize_file_name($active_file);
    ...
    public static function detect_context()
    {
        ...
        $is_login =
            ...
            (isset($_GET["vaptsecure_test_context"]) &&
                $_GET["vaptsecure_test_context"] === "login");
        ...
        add_action(
            ...
            function () {
                ...
                foreach (self::$feature_configs as $feature_key => $cfg) {
                    // Enforce Scope Logic
                    if ($cfg["scope"] === "login" && !$context["is_login"]) {
                        continue;
                    }
