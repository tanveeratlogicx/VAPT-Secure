...
class VAPT_SECURE_Admin
{
  ...
  public function __construct()
  {
    ...
    add_action('wp_ajax_vapt_secure_check_progress', array($this, 'ajax_check_progress'));
    add_action('wp_ajax_vapt_secure_process_scan', array($this, 'ajax_process_scan'));
...
?>
  ...
  public function ajax_process_scan()
  {
    ...
    global $wpdb;
    $target_url = $wpdb->get_var($wpdb->prepare("SELECT target_url FROM {$wpdb->prefix}vapt_secure_scans WHERE id = %d", $scan_id));
  ...
  public function ajax_check_progress()
  {
    ...
    global $wpdb;
    $scan = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}vapt_secure_scans WHERE id = %d", $scan_id), ARRAY_A);

    if (!$scan) {
      wp_send_json_error('Scan not found');
    }

    $progress = get_option('vapt_secure_scan_progress_' . $scan_id);
    ...
    // Get found count
    $found_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}vapt_secure_scan_results WHERE scan_id = %d", $scan_id));
  ...
  private function get_vulnerability_data()
  {
    if ($this->vulnerabilities_cache === null) {
      ...
      $active_file = VAPT_SECURE_ACTIVE_DATA_FILE;
      $file_path = VAPT_SECURE_PATH . 'data/' . sanitize_file_name($active_file);
  ...
  public function handle_sync_processing()
  {
    ...
    $target_url = $wpdb->get_var($wpdb->prepare(
      "SELECT target_url FROM {$wpdb->prefix}vapt_secure_scans WHERE id = %d",
      $scan_id
    ));
  ...
  public function admin_page()
  {
    ...
    if (isset($_POST['start_scan']) && wp_verify_nonce($_POST['vapt_secure_scan_nonce'], 'start_scan')) {
    ...
    if (isset($_GET['vapt_secure_sync_done'])) {
    ...
    ?>
    <div class="wrap">
      <h1>VAPT Security Auditor <span class="vapt-version-badge">v<?php echo VAPT_SECURE_AUDITOR_VERSION; ?></span></h1>

      <div class="vapt-scan-form">
        <h2>Start New Scan</h2>
        <form method="post" action="">
          <?php wp_nonce_field('start_scan', 'vapt_secure_scan_nonce'); ?>
        ...
        <div class="vapt-scan-results">
          <div class="vapt-results-header">
            ...
            <div style="margin-bottom: 10px; display: flex; align-items: center; gap: 10px;">
              ...
              <?php
              $is_sync = isset($_GET['sync_process']) || isset($_GET['vapt_secure_sync_done']);
              if ($is_sync) {
          ...
          <script type="text/javascript">
            var vapt_secure_scan_results = <?php echo json_encode($scan_results ?: []); ?>;
          </script>
          ...
          if ($status === 'running' || $status === 'pending') {
            ...
            $progress = get_option('vapt_secure_scan_progress_' . $scan_data['id']);
            $checked = $progress['checked'] ?? 0;
            ...
            $found_count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}vapt_secure_scan_results WHERE scan_id = %d", $scan_data['id']));
              ...
              jQuery(document).ready(function($) {
                ...
                function processScanBatch(offset) {
                    ...
                    $.ajax({
                        ...
                        type: 'POST',
                        data: {
                            action: 'vapt_secure_process_scan',
                            scan_id: vaptScanId,
                            offset: offset
                        },
                ...
                function checkProgress() {
                    $.ajax({
                        url: ajaxurl,
                        data: {
                            action: 'vapt_secure_check_progress',
                            scan_id: vaptScanId
                        },
                        success: function(response) {
                      ...
                      <tr class="vapt-result-row" data-severity="<?php echo esc_attr($severity); ?>" data-title="<?php echo esc_attr($vuln_info['name'] ?? ''); ?>" data-id="<?php echo esc_attr($result['vulnerability_id']); ?>">
                        ...
                        <td>
                          ...
                          <?php if (!empty($result['screenshot_path'])): ?>
                            <a href="<?php echo esc_url(VAPT_SECURE_URL . 'data/screenshots/' . basename($result['screenshot_path'])); ?>" target="_blank" class="button" title="View Evidence Screenshot"><span class="dashicons dashicons-visibility"></span></a>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
              ...
              try {
                ...
                if (Array.isArray(paths) && paths.length > 0) {
                  ...
                  paths.forEach(function(path) {
                    ...
                    var screenshotUrl = '<?php echo esc_url(VAPT_SECURE_URL); ?>data/screenshots/' + screenshotName;
